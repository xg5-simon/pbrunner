...
def export_watchlist(cb, parser, args):
    if args.export_all:
        watchlists = cb.select(Watchlist)
        for watchlist in watchlists:
            exported = {'watchlist': watchlist._info, 'reports': [report._info for report in watchlist.reports]}
            print(f'Exporting {watchlist.name}.json')
#            print(json.dumps(exported))
            with open(f'{watchlist.name}.json', 'w') as f:
        ...
        all_watchlists = []
        for watchlist in watchlists:
            all_watchlists.append(watchlist.name)

        cb_watchlists = [
            'AMSI Threat Intelligence',
            'ATT&CK Framework',
            'Carbon Black Advanced Threats',
            'Carbon Black AlienVault',
            ...
            'Carbon Black Facebook ThreatExchange',
            'Carbon Black SANS',
            'Carbon Black TOR',
            'Tor Exit Nodes',
            'US Cybercom Malware Alert'
        ]

        watchlist_custom = set(all_watchlists) - set(cb_watchlists)
        for watchlist in watchlist_custom:
            watchlist = get_watchlist(cb, watchlist_name=watchlist)
            exported = {'watchlist': watchlist._info, 'reports': [report._info for report in watchlist.reports]}
            print(f'Exporting {watchlist.name}.json')
#            print(json.dumps(exported))
            with open(f'{watchlist.name}.json', 'w') as f:
...
def import_watchlist(cb, parser, args):
    if args.bulk_import:
 #       print(os.listdir(args.bulk_import))
        watchlist_path = args.bulk_import
        for file_name in [file for file in os.listdir(watchlist_path) if file.endswith('.json')]:
            print(f'Importing \"{file_name}\"')
            with open(watchlist_path + file_name) as json_file:
...
def main():
    ...
    specifier.add_argument("-x", "--export_all", action="store_true", help="Export all watchlists")
    specifier.add_argument("-c", "--custom_all", action="store_true", help="Export all custom watchlists")

    import_command = commands.add_parser("import", help="Import a previously exported watchlist")
    import_command.add_argument("-b", "--bulk_import", type=str, help="Import all watchlist files from the current "
                                                                      "working directory")
